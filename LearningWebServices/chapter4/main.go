...
func handleUpdateList(w http.ResponseWriter, r *http.Request) {
	...
	for i, list := range allData {
		if strconv.Itoa(list.ID) == id {
			var updateList ShoppingList
			err := json.NewDecoder(r.Body).Decode(&updateList)
			if err != nil {
				http.Error(w, err.Error(), http.StatusBadRequest)
				return
			}
			allData[i] = updateList

			if err := json.NewEncoder(w).Encode(updateList); err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
			return
